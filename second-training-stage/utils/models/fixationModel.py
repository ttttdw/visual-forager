...
class Actor(nn.Module):
    def __init__(self, block_num, head_num):
        ...
        self.head = nn.Sequential(nn.LayerNorm(128),
                                  nn.Linear(128, 1))
        # self.head1 = nn.Sequential(nn.LayerNorm(128),
        #                            nn.Linear(128, 1), nn.Sigmoid())
        # self.head2 = nn.Sequential(nn.LayerNorm(256),
        #                            nn.Linear(256, 128), nn.Linear(128, 1))
    ...
    def forward(self, attention_map, values):
        values = (values - 8) / 4.6
        values = f.relu(
            self.fc4(f.relu(self.fc3(f.relu(self.fc2(f.relu(self.fc1(values))))))))
...
class ViT(nn.Module):
    ...
    def __init__(self, img_size, patch_size, num_hiddens, mlp_num_hiddens,
        ...
        self.value_token = nn.Parameter(torch.zeros(1, 1, num_hiddens))
        num_steps = self.patch_embedding.num_patches + 2  # Add the cls token
    ...
    def forward(self, X):
        ...
        X = torch.cat((self.value_token.expand(X.shape[0], -1, -1), X), 1)
