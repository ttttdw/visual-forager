...
def visual_process(observation, model_stimuli, mean, std, pe):
    ...
    output_stimuli = model_stimuli(observation)
...
def generate_ior_map(fixations, size, fogetting=0.8):
    if len(fixations) == 1:
        return torch.ones((1, size, size)).to(device), None
    ior_map = torch.ones((1, size, size))
    revers_ior = torch.zeros((1, size, size))
    for f in fixations[:-1]:
        revers_ior = revers_ior * fogetting
        revers_ior[0, f[1], f[0]] = 1
        # ior_map[0, f[1], f[0]] = 0
...
def get_attention_map(
    ...
    ior_map, ior_img = generate_ior_map(fixations, size, forgetting)
...
def get_eccattention_map(
    ...
    ior_map, ior_img = generate_ior_map(fixations, size, fogetting=forgetting)
    ...
    # saccade_img.save("saccade_img.jpg")
    attention_map = torch.cat(attention_maps).unsqueeze(0)
    # pos_embed = torch.zeros((1, 1, 32, 32))
    
    # for i in range(32):
    #     for j in range(32):
    #         # You can use any kind of encoding; here's a simple one
    #         pos_embed[0, 0, i, j] = math.sin(i-16) + math.cos(j-16)
    # pos_embed = pos_embed[
    #     :,:,
    #     (size - fixations[-1][1] - 1): (2 * size - fixations[-1][1] - 1),
    #     (size - fixations[-1][0] - 1): (2 * size - fixations[-1][0] - 1),
    # ]

    return attention_map# + 0.1 * pos_embed.to(device)

def get_perfect_map(observation,
    fixations,
    env_config,
    forgetting=0.8):
    size = env_config["variable"]["size"]
    ...
    ior_map, ior_img = generate_ior_map(fixations, size, forgetting)
    for i in range(4):
        attention_map = observation[i, :, :]
        attention_map = attention_map * ior_map
        attention_map = (attention_map - attention_map.mean()) / \
            (attention_map.std()+1e-5)
        attention_maps.append(attention_map)
    attention_map = torch.cat(attention_maps).unsqueeze(0)
    return attention_map