...
class GridVisualForagingEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, task_mode=1):
        ...
        # Stimuli file list
        folder_path = 'visual_foraging_gym/envs/Train'
    ...
    def _get_info(self):
        return {'value': self.rewards[self.task_mod], 'click_count': self.clicks, 'now click': self.now_click, 'filename': self.image_file_set}
    ...
    def reset(self, seed=None, option=None):
        ...
        super().reset(seed=rand_seed)

        random.shuffle(self.rewards[self.task_mod])

        p1 = np.random.randint(0, 32)
        p2 = np.random.randint(0, 32-p1)
        p3 = np.random.randint(0, 32-p1-p2)
        p4 = np.random.randint(0, 32-p1-p2-p3)
        ...
        # Read stimulis
        folder_path = 'visual_foraging_gym/envs/Train'
        self.image_file_set = random.sample(self.image_file_list, 13)
        # self.image_file_set[0] = "045.jpg"
        # self.image_file_set[1] = "8055181.thl.jpg"
        # self.image_file_set[2] = "8059226.thl.jpg"
        # self.image_file_set[3] = "8077486.thl.jpg"
        image_set = []
        for file in self.image_file_set:
            image = Image.open(folder_path + '/' + file)
            image = image.resize((self.pix_size, self.pix_size))
            image = np.array(image)
            image = torch.from_numpy(image).to(self.device)
            image_set.append(image)
        self.target_set = image_set[0:4]
        self.distractor_set = image_set[4:14]
        distractor_set = []
