...
class GridVisualForagingEnv(gym.Env):
    ...
    def __init__(self, render_mode="rgb_array", task_mode=1, penalty=-5, fixation_bonus=0):

        self.SCORE = 0

        self.rewards = [[4, 4, 4, 4], [2, 4, 8, 16], [2, 4, 8, 16]]
        self.penalty = penalty  # the reward get when click distractor
        # the bonus get when look at the correct item
        self.fixation_bonus = fixation_bonus
    ...
    def seed(self, seed=None):
        # Set the environment's internal seed
        if seed is not None:
            np.random.seed(seed)
            random.seed(seed)
    ...
    def reset(self, seed=None, option=None):
        ...
        p2 = random.randint(0, 32-p1)
        p3 = random.randint(0, 32-p1-p2)
        p4 = random.randint(0, 32-p1-p2-p3)
        self.popularities[self.task_mod] = [p1, p2, p3, 32-p1-p2-p3]
        random.shuffle(self.popularities[self.task_mod])
    ...
    def step(self, action):
        ...
        if click_flag:
            ...
            reward = 0
            if any(np.all(action == vector) for vector in self.target_ones):
                reward = self.rewards[self.task_mod][0] * self.fixation_bonus
            elif any(np.all(action == vector) for vector in self.target_twos):
                reward = self.rewards[self.task_mod][1] * self.fixation_bonus
            elif any(np.all(action == vector) for vector in self.target_threes):
                reward = self.rewards[self.task_mod][2] * self.fixation_bonus
            elif any(np.all(action == vector) for vector in self.target_fours):
                reward = self.rewards[self.task_mod][3] * self.fixation_bonus
            elif any(np.all(action == vector) for vector in self.distractor_positions):
                reward = 0
            else:
                reward = 0
