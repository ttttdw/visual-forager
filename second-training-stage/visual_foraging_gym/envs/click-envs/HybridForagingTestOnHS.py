...
class GridVisualForagingEnv(gym.Env):
    ...
    def _get_perfect_obs(self):
        canvas = torch.zeros((4, self.size, self.size), device=self.device)
        for position in self.target_ones:
            canvas[0, position[1], position[0]] = 1
        for position in self.target_twos:
            canvas[1, position[1], position[0]] = 1
        for position in self.target_threes:
            canvas[2, position[1], position[0]] = 1
        for position in self.target_fours:
            canvas[3, position[1], position[0]] = 1
        return canvas
    ...
    def _get_info(self):
        ...
        information = {'value': self.rewards, 'click_count': self.clicks, 'now click': self.now_click,
                       'filename': self.target_image_file_list, 'click list': self.click_list, 'radius score': self.radius_score}
    ...
    def reset(self, seed=None, option=None, target_image_file_list=None, distractor_image_file_list=None, distractor_index=None, all_sprited_positions=None, values=None, popularities=None):
        ...
        points = np.concatenate((np.ones(self.popularities[3])*self.rewards[3], np.ones(
            self.popularities[2])*self.rewards[2], np.ones(self.popularities[1])*self.rewards[1], np.ones(self.popularities[0])*self.rewards[0]))
        points = np.sort(points)[::-1]
        self.upperbound = sum(points[:20])
        ...
        self.radius_score = self.get_radius_score()
    ...
    def get_radius_score(self):
        # degree to radius
        degree = np.array([1.5, 2, 4, 8])
        distance = 750
        screen_width = 300
        screenY = 1080
        degree = degree * math.pi /180
        radius = np.tan(degree) * distance * screenY / screen_width
        # score in radius
        score = [0, 0, 0, 0]
        targets = [self.target_ones, self.target_twos, self.target_threes, self.target_fours]
        for j in range(4):
            r = radius[j]
            for i, target in enumerate(targets):
                for vector in target:
                    position = (vector + 0.5) * self.pix_size
                    if np.linalg.norm(position - np.array(self.fixations[-1])) < r:
                        score[j] += self.rewards[i]
            for vector in self.distractor_positions:
                position = (vector + 0.5) * self.pix_size
                if np.linalg.norm(position - np.array(self.fixations[-1])) < r:
                    score[j] += -1
    ...
    def step(self, action):
        ...
        if click_flag:
            ...
            if any(np.all(action == vector) for vector in self.target_ones):
                ...
                self.distractor_set = [self.distractor_set[i] for i, arr in enumerate(
                    self.distractor_positions) if not np.array_equal(arr, action)]
                self.distractor_positions = [
                    arr for arr in self.distractor_positions if not np.array_equal(arr, action)
                ]
        ...
        if len(self.target_ones)+len(self.target_twos)+len(self.target_threes)+len(self.target_fours) < 1:
            ...
            terminated = False
        if self.time < 0 or self.clicked > 19:
            truncated = True
            # if self.SCORE > 200:
            #     reward += 100
        else:
...
class PerfectObservationEnv(GridVisualForagingEnv):
    def _get_obs(self):
        canvas = torch.zeros((4, self.size, self.size), device=self.device)
        for position in self.target_ones:
            canvas[0, position[1], position[0]] = 1
