...
class GridVisualForagingEnv(gym.Env):
    ...
    def __init__(self, render_mode="rgb_array", rewards=None):
    ...
    def _get_info(self):
        return {'value': self.rewards, 'click_count': self.clicks, 'now click': self.now_click, 'filename': self.target_image_file_list}
    ...
    def reset(self, seed=None, option=None):
        ...
        self.popularities = [p1, p2, p3, 32-p1-p2-p3]
        random.shuffle(self.popularities)
    ...
    def step(self, action):
        ...
        if click_flag:
            ...
            if any(np.all(action == vector) for vector in self.target_ones):
                reward = self.rewards[0] / 10
            elif any(np.all(action == vector) for vector in self.target_twos):
                reward = self.rewards[1] / 10
            elif any(np.all(action == vector) for vector in self.target_threes):
                reward = self.rewards[2] / 10
            elif any(np.all(action == vector) for vector in self.target_fours):
                reward = self.rewards[3] / 10
            elif any(np.all(action == vector) for vector in self.distractor_positions):
                reward = -1 / 10
            else:
                reward = -0.1
