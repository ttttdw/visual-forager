...
from visual_foraging_gym.envs.grid_render_objects import (
    Target,
    TargetOne,
    TargetTwo,
    TargetThree,
...
class GridVisualForagingEnv(gym.Env):
    ...
    def __init__(
        ...
        self.popularities = [[2, 4, 8, 16], [8, 8, 8, 8], [16, 8, 4, 2]]
        self.task_mod = task_mode  # random.choice([0, 1, 2])
        self.rewards[self.task_mod] = values
        self.popularities[self.task_mod] = popularity
        ...
        self.fixations = [
            [
                (round((self.size - 1) / 2) + 0.5) * self.pix_size,
                (round((self.size - 1) / 2) + 0.5) * self.pix_size,
            ]
        ]
        ...
        self.score = Score(self.SCORE, self.font, self.SCREEN_WIDTH, self.SCREEN_HEIGHT)
    ...
    def _get_obs(self):
        if self.render_mode == "human":
            ...
            # Make new canvas
            canvas = torch.ones(
                (self.size * self.pix_size, self.size * self.pix_size, 3),
                device=self.device,
            )
            canvas = canvas * 128
    ...
    def _get_info(self):
        return {
            "value": self.rewards[self.task_mod],
            "click_count": self.clicks,
            "now click": self.now_click,
            "filename": self.target_image_file_list,
            "click list": self.click_list,
        }
    ...
    def _add_target_one(self):
        ...
        self.all_sprited_positions = [
            arr
            for arr in self.all_sprited_positions
            if not np.array_equal(arr, position)
        ]
    ...
    def _new_patch(self):
        ...
        while len(self.all_sprited_positions) < self.n_item:
            ...
            if not any(
                np.all(position == vector) for vector in self.all_sprited_positions
            ):
