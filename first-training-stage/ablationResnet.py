...
from utils.models.clickModelAblation import ActorResNet, TaskEmbedding
from utils.gen_ppo_memory import Memory
from TestLargePPOAblation import test
from TestLargePPOAblationShuffle import test as test_shuffle
...
episode_saved = 0
CHECK_POINT_PATH = "data/model/ablation-resnet.pt"
...
for i_episode in range(config["ppo"]["num_episode"]):
    ...
    # _, test_score2, _, _ = test(CHECK_POINT_PATH, actor, test_env_2, vgg_model)
    # TestScore2.append(test_score2[0])

    # _, test_score3, _, _ = test(CHECK_POINT_PATH, actor, test_env_3, vgg_model)
    # TestScore3.append(test_score3[0])

    # _, test_score_g, _, _ = test(CHECK_POINT_PATH, actor, test_env_g, vgg_model)
    # TestScoreG.append(test_score_g[0])
    # _, test_score_s, _, _ = test_shuffle(CHECK_POINT_PATH, actor, test_env, vgg_model)
