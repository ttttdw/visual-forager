# self attention, value embedding
import torch
...
class TaskNoEmbedding(TaskEmbedding):
    def __init__(self):
        super(TaskNoEmbedding, self).__init__()
...
class ActorResNet(Actor):
    def __init__(self):
        ...
        self.actor = resnet.layer1
        self.pooling = resnet.avgpool
        ...
        self.head = nn.Sequential(nn.LayerNorm(256),
                                  nn.Linear(256, 1))
    ...
    def forward(self, attention_map, values):
        ...
        attention_map = attention_map + values
        attention_map = f.relu(self.conv(attention_map))

        out = self.actor(attention_map)
        value = self.pooling(out).view(-1, 256)
...
class ViTBlock(nn.Module):
    def __init__(self, num_hiddens, norm_shape, mlp_num_hiddens,
        ...
        super().__init__()
        self.ln1 = nn.LayerNorm([257, norm_shape])
        self.attention = nn.MultiheadAttention(num_hiddens, num_heads,
                                               dropout, batch_first=True)
        self.ln2 = nn.LayerNorm([257, norm_shape])
