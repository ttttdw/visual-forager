...
from utils.models.gen_actor_critic19 import Actor, TaskEmbedding
...
def test(path, env, vgg_model, scale=20):
    ...
    for i in range(scale * 1):
        ...
        # values = torch.tensor(info["value"]).float().unsqueeze(0).to(device)
        action, _, _, _ = select_action(actor, task_embedding, attention_map, values)
        b, a = divmod(int(action.cpu()), size)
        saccades.append([a - fixations[-1][0], b - fixations[-1][1]])
        fixations.append([a, b])
        observation, reward, terminated, truncated, info = env.step(np.array([a, b]))
        ...
        if terminated or truncated:
            ...
            click_count["click_one"].append(info["click_count"]["click_target_one"])
            click_count["click_two"].append(info["click_count"]["click_target_two"])
            click_count["click_three"].append(info["click_count"]["click_target_three"])
            click_count["click_four"].append(info["click_count"]["click_target_four"])
...
if __name__ == "__main__":
    ...
    data = {
        ...
        "click observer": click_observer.tolist(),
    }
